    ...
    fn get_api_key(&self) -> Result<String, ProviderError> {
        ...
        std::env::var("META_API_KEY")
            .map_err(|_| ProviderError::AuthError(
                "META_API_KEY not set. Use /set-key meta <key> or set environment variable".to_string()
            ))
    }

    fn get_base_url(&self) -> String {
        self.base_url.clone()
            .unwrap_or("https://api.llama-api.com/chat/completions".to_string())
    }
...
impl Provider for MetaProvider {
    ...
    fn generate_with_system(&self, system: &str, context: &str, config: &crate::state::Config) -> Result<String, ProviderError> {
        ...
        let model = if config.model.starts_with("gpt") || config.model.starts_with("claude") {
