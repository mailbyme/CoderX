...
impl HunyuanProvider {
    ...
    fn get_api_key(&self) -> Result<String, ProviderError> {
        ...
        std::env::var("HUNYUAN_API_KEY")
            .map_err(|_| ProviderError::AuthError(
                "HUNYUAN_API_KEY not set. Use /set-key hunyuan <key> or set environment variable".to_string()
            ))
    }

    fn get_base_url(&self) -> String {
        self.base_url.clone()
            .unwrap_or("https://hunyuan.tencentcloudapi.com/v1/chat/completions".to_string())
    }
...
impl Provider for HunyuanProvider {
