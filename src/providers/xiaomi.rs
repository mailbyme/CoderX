...
impl XiaomiProvider {
    ...
    fn get_api_key(&self) -> Result<String, ProviderError> {
        ...
        std::env::var("XIAOMI_API_KEY")
            .map_err(|_| ProviderError::AuthError(
                "XIAOMI_API_KEY not set. Use /set-key xiaomi <key> or set environment variable".to_string()
            ))
    }

    fn get_base_url(&self) -> String {
        self.base_url.clone()
            .unwrap_or("https://api.xiaomy.com/v1/chat/completions".to_string())
    }
